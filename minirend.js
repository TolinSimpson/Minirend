...
/**
 * minirend - Development Server & Build Configurator
 * --------------------------------------------------
 * Run with: node minirend.js
 * 
...
function handleBootstrap(req, res) {
  ...
  const env = { ...process.env, MINIREND_BOOTSTRAP_ONLY: "1" };
...
function handleBuild(req, res) {
  ...
  req.on("end", () => {
    ...
    const outName = (payload.outName || "minirend").trim() || "minirend";

    const configText = [
      "# minirend build config",
      "# Generated by minirend.js configurator",
      "",
      `APP_DIR=${appDir}`,
      `OUT_NAME=${outName}`,
      "",
...
server.listen(PORT, () => {
  ...
  console.log("  ┌─────────────────────────────────────────┐");
  console.log("  │                                         │");
  console.log("  │   minirend Development Server           │");
  console.log("  │                                         │");
