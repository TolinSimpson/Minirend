...
REM Wrapper for Cosmopolitan archiver on Windows
REM Invokes cosmoar through bash since it's a shell script

set "SCRIPT_DIR=%~dp0"
set "ROOT=%SCRIPT_DIR%.."
set "COSMO_BIN=%ROOT%\cosmocc\bin"
set "COSMOAR=%COSMO_BIN%\cosmoar"
...
REM Run cosmoar directly through bash, passing all arguments
"!BASH_EXE!" "%COSMOAR%" %*
exit /b %ERRORLEVEL%
